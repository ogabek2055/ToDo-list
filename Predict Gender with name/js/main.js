...
let predictGender = () => {
  ...
  // check input field is not empty and the entered name does not contain anything but alphabit.
  if (name.length > 0 && /^[A-Za-z] +$/.test(name)) {
    fetch(finalURL)
      ...
      .then(data => {
        ...
        <h1" id="gender">${data.gender}</h1><h4 id="prob">Probability:${data.probability}</h4>`;
...
document
  ...
  .addEventListener("click", predictGender);
